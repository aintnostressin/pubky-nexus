...
- *all*: Every root post. Once nexusd's `trust-recompute` job has built a trust ranking, posts by authors absent from it are hidden. Without a ranking the stream is served unfiltered.
