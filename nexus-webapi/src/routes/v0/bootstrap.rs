...
#[utoipa::path(
    ...
    description = "Initial payload for all data required to bootstrap the pubky.app application. The client app will request it while the user is performing sign-in/sign-up in order to pre-populate the client DB. The post timeline is the `source=all` stream: once a trust ranking exists it hides posts by unranked authors.",
    tag = "Bootstrap",
    params(
