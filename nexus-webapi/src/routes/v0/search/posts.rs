...
mod tests {
    ...
    fn parse_query(s: &str) -> std::result::Result<SearchPostsByContentQuery, serde_urlencoded::de::Error> {
    ...
    fn kind_unknown_parses_as_unknown() {
        let q = parse_query("q=bitcoin&kind=not-a-kind")
            .expect("lenient kind parsing must not error");
