...
impl VariantProcessor for ImageProcessor {
    ...
    async fn process(
        ...
        let child_output = Command::new("convert")
            .arg("-limit").arg("memory").arg(LIMIT_MEMORY)
            .arg("-limit").arg("map").arg(LIMIT_MAP)
            .arg("-limit").arg("disk").arg(LIMIT_DISK)
            .arg("-limit").arg("area").arg(LIMIT_AREA)
            .arg("-limit").arg("width").arg(LIMIT_WIDTH)
            .arg("-limit").arg("height").arg(LIMIT_HEIGHT)
            .arg("-limit").arg("time").arg(LIMIT_TIME)
            .arg(origin_file_path)
            .arg("-coalesce")
            .arg("-resize").arg(format!("{}x", options.width))
