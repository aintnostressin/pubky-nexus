...
pub struct ApiConfig {
    ...
    /// Hide root posts by authors absent from the trust ranking on shared
    /// surfaces (`source=all` streams, tag and content search, global hot
    /// tags). Off reproduces the unfiltered behaviour exactly. Fails open:
    /// has no effect until a ranking exists.
