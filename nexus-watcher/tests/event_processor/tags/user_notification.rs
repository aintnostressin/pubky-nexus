...
async fn test_homeserver_put_tag_user_notification() -> Result<()> {
    ...
    let tagged_user = PubkyAppUser::new("Watcher:PutTagNotification:TaggedUser".to_string(), Some("test_homeserver_put_tag_user_notification".to_string()), None, None, None);
    let tagged_user_id = test.create_user(&tagged_kp, &tagged_user).await?;

    // Create the second user (tagger)
    let tagger_kp = Keypair::random();

    let tagger_user = PubkyAppUser::new("Watcher:PutTagNotification:TaggerUser".to_string(), Some("test_homeserver_put_tag_user_notification".to_string()), None, None, None);
