...
async fn test_edit_parent_post_notification() -> Result<()> {
    ...
    let user_a = PubkyAppUser::new("Watcher:ReplyParentEditNotification:UserA".to_string(), Some("User A bio".to_string()), None, None, None);
    ...
    let user_b = PubkyAppUser::new("Watcher:ReplyParentEditNotification:UserB".to_string(), Some("User B bio".to_string()), None, None, None);
